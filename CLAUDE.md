...
- Concepts are always listed alphabetically
- Markdown editor is simple: raw code input + rendered preview toggle; no WYSIWYG
- "Cancel" discards unsaved changes and returns to rendered view
- References field only appears post-creation (not during the creation form)
- The app is frontend-only — no API calls, no authentication, no persistence beyond browser state (localStorage may be used for persistence across sessions)
